...
Locations = ['Shanghai', 'Beijing', 'Guangzhou', 'Hangzhou', 'Chengdu', 'Xiangang', 'Taiwan', 'China', 'Nanjing',
             'Jinan', 'Xian']
Company = ['Zhejiang University', 'Shanghai University', 'Beijing University', 'Shandong University', 'SJTU',
           'Nanjing University'
    , 'Tsinghua University', 'Dongbei University', 'Alibaba', 'Tencent', 'Baidu', 'Mobike', 'Zhihu', 'Bilibili',
           'Wangyi']
...
def create_name():
    ...
    if random.randint(0, 1):
        Chinese_name = xing[x] + ming[m1]
        print('生成了用户中文名为: ', Chinese_name)
    else:
        m2 = random.randint(0, len(ming))
        Chinese_name = xing[x] + ming[m1] + ming[m2]
        print('生成了用户中文名为: ', Chinese_name)

    user_name = ''.join(['' + x[0] for x in pypinyin.pinyin(Chinese_name, 0)])
    length = len(user_name)
    upper_index = random.sample(range(length), random.randint(0, length))
    for i in upper_index:
        user_name = user_name.replace(user_name[i], user_name[i].upper())
    added_number = random.sample(user_name, 4)
    user_name += ''.join(added_number)
    print('生成的英文用户名为：', user_name)
    return user_name
...
def create_profile():
    ...
    return Chinese_name, location, company
